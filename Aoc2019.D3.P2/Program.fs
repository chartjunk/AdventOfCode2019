...
let firstIntersection (ms & [w1ms; w2ms]) =
    ...
    let ((_, w1fresid), w1fw1ix, w1fw2ix) = minIxs fst
    let ((_, w2fresid), w2fw1ix, w2fw2ix) = minIxs snd
    let w1fs = cumulativeLength (w1fw1ix, w1ms) + cumulativeLength (w1fw2ix, w2ms) + w1fresid
    let w2fs = cumulativeLength (w2fw1ix, w1ms) + cumulativeLength (w2fw2ix, w2ms) + w2fresid
    Math.Min(w1fs, w2fs)
